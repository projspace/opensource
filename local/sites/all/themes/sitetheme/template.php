...
function sitetheme_preprocess_node(&$vars) {
  ...
  elseif (($vars['node']->type == 'post' || $vars['node']->type == 'podcast') || $vars['node']->type == 'video' && !$vars['page']) {
  ...
  elseif (($vars['node']->type == 'post' || $vars['node']->type == 'podcast') || $vars['node']->type == 'podcast') && $vars['page']) {
  ...
  $vars['node_license'] = '';
  if ($vars['node']->type == 'post' || $vars['node']->type == 'postcast') || $vars['node']->type == 'video') && $vars['page']) {
...
function sitetheme_quicktabs_tabs($quicktabs, $active_tab = 'none') {
  ...
  foreach ($quicktabs['tabs'] as $i => $tab) {
    ...
    //if (!empty($quicktabs['title'])) {
    //  $attributes_li = drupal_attributes(array('class' => $class, 'onclick' => 'trackTab(\''. $tab['title'] .'\')'));
    //}
    //else {
