...
function sitetheme_preprocess_node(&$vars) {
  ...
  elseif ($vars['node']-> type == 'article' && !$vars['page']) {
    ...
    $vars['links_all'] .= '<span class="article-author-info">'. t('Posted !date by !author', array('!date' => format_date($vars['node']->created, 'custom', 'j M Y'), '!author' => l($vars['node']->name, 'user/'. $vars['node']->uid))) .'<a href="/user/'. $vars['node']->uid .'/feed" class="article-author-feed"></a></span>';
 ...
 * @see theme_comment()
 */
function sitetheme_preprocess_comment(&$variables) {
