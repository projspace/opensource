...
function opensource_preprocess_username(&$vars) {
  ...
  if (count($mail) == 2 && strcasecmp($mail[1], 'redhat.com') == 0) {
    if(isset($vars['link_path'])) {
      ...
      $username = isset($account->field_display_name[LANGUAGE_NONE][0]['value']) ? l($account->field_display_name[LANGUAGE_NONE][0]['value'], 'user/' . $account->uid, array('attributes' => array('class' => array('username')))) : l($vars['name'], '');
    }
  ...
  if($vars['element']['#field_name'] == 'field_default_license') {
    switch ($vars['element']['#object']->field_default_license[LANGUAGE_NONE][0]['value']) {
      ...
      case 'CC-BY 3.0':
        ...
        break;
      case 'CC0':
              $vars['items']['0']['#markup'] = '<a rel="license" href="http://creativecommons.org/publicdomain/zero/1.0/">
        <img alt="Creative Commons License" style="border-width:0" src="'. base_path() . path_to_theme() .'/images/CC0.png" title="This work is licensed under a CC0 1.0 Universal (CC0 1.0) Public Domain Dedication license" /></a>';
        break;
      case 'None':
              $vars['items']['0']['#markup'] = '';
        break;
...
function opensource_preprocess_search_result(&$vars) {
  $allowed_types = array('article', 'event', 'resource', 'poll', 'page', 'group_page');
...
function opensource_preprocess_html(&$variables, $hook) {
  if(arg(0) == 'node' && arg(1) != NULL) {
    ...
    if ($node->type == 'article' && $node->status == 0) {
